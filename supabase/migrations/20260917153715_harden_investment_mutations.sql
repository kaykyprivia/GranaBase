...
begin
  ...
  if trim(p_investment_type) not in (
    'Reserva', 'Tesouro', 'CDB', 'FII',
    'ETF', 'Acao', 'Crypto', 'Outro'
  ) then
    raise exception 'Tipo do investimento invalido';
  end if;
...
-- Fechar mutacoes diretas: todas as alteracoes passam pelas RPCs acima.
revoke insert, update, delete on table public.investments from anon, authenticated;
