-- Business sale delivery method / fulfillment model.

alter table public.business_sales
  add column if not exists delivery_method text not null default 'UNSPECIFIED';

alter table public.business_sales
  drop constraint if exists business_sales_delivery_method_check;

alter table public.business_sales
  add constraint business_sales_delivery_method_check
  check (
    delivery_method in (
      'UNSPECIFIED',
      'OWN_DELIVERY',
...
alter table public.business_sales
  drop constraint if exists business_sales_customer_pickup_financials_check;

alter table public.business_sales
  add constraint business_sales_customer_pickup_financials_check
  check (
    delivery_method <> 'CUSTOMER_PICKUP'
    or (delivery_fee = 0 and delivery_cost = 0)
  );

comment on column public.business_sales.delivery_method is
  'How the order reached the customer: own delivery, courier/app, customer pickup, shipping carrier, other or unspecified.';

-- Delivery-method-aware sale creation.
drop function if exists public.create_business_sale(uuid, jsonb, text, uuid, timestamptz, text, boolean, text, numeric, numeric);
...
create function public.create_business_sale(
  ...
  p_delivery_method text default 'UNSPECIFIED'
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  normalized_delivery_method text := upper(trim(coalesce(p_delivery_method, 'UNSPECIFIED')));
begin
  ...
  if normalized_delivery_method not in ('UNSPECIFIED', 'OWN_DELIVERY', 'COURIER_APP', 'CUSTOMER_PICKUP', 'SHIPPING_CARRIER', 'OTHER') then
    raise exception 'Modalidade de entrega invalida';
  end if;

  if normalized_delivery_method = 'CUSTOMER_PICKUP' and (delivery_fee_value <> 0 or delivery_cost_value <> 0) then
    raise exception 'Retirada pelo cliente nao pode ter taxa ou custo de entrega';
  end if;
...
-- Delivery-method-aware sales filters.
drop function if exists public.get_business_sales_page(uuid, integer, integer, text, text, date, date, text, text);
...
create function public.get_business_sales_page(
  ...
  p_delivery_method text default 'all'
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  normalized_delivery_method text := upper(trim(coalesce(p_delivery_method, 'all')));
  total_count bigint;
  page_ids jsonb;
begin
  if current_user_id is null then
  ...
  if normalized_delivery_method not in ('ALL', 'UNSPECIFIED', 'OWN_DELIVERY', 'COURIER_APP', 'CUSTOMER_PICKUP', 'SHIPPING_CARRIER', 'OTHER') then
    raise exception 'Filtro de modalidade de entrega invalido';
  end if;
  ...
  where sale.user_id = current_user_id
    ...
    and (
      normalized_delivery_method = 'ALL'
      or sale.delivery_method = normalized_delivery_method
    )
    and (
...
drop function if exists public.get_business_sales_summary(uuid, text, text, date, date, text, text);
