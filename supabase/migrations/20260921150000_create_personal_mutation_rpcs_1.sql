-- ============================================================
-- PERSONAL MUTATION RPCs - Parte 1: income_entries + expense_entries
-- ============================================================

-- ============================================================
-- INCOME ENTRIES
-- ============================================================

create or replace function public.create_income_entry(p_payload jsonb)
returns uuid
language plpgsql
security definer
set search_path = ''
as $$
declare
  current_user_id uuid := auth.uid();
  new_id uuid;
begin
  if current_user_id is null then
    raise exception 'Usuario nao autenticado';
  end if;

  insert into public.income_entries (
    user_id, description, amount, category, received_at, payment_method, notes
  )
  values (
    current_user_id,
    p_payload->>'description',
    (p_payload->>'amount')::numeric,
    p_payload->>'category',
    (p_payload->>'received_at')::date,
    p_payload->>'payment_method',
    p_payload->>'notes'
  )
  returning id into new_id;

  return new_id;
end;
$$;

revoke all on function public.create_income_entry(jsonb)
  from public, anon;

grant execute on function public.create_income_entry(jsonb)
  to authenticated;


create or replace function public.update_income_entry(
  p_id uuid,
  p_payload jsonb
)
returns void
language plpgsql
security definer
set search_path = ''
as $$
declare
  current_user_id uuid := auth.uid();
begin
  if current_user_id is null then
    raise exception 'Usuario nao autenticado';
  end if;

  update public.income_entries
  set
    description = coalesce(p_payload->>'description', description),
    amount = coalesce((p_payload->>'amount')::numeric, amount),
    category = coalesce(p_payload->>'category', category),
    received_at = coalesce((p_payload->>'received_at')::date, received_at),
    payment_method = coalesce(p_payload->>'payment_method', payment_method),
    notes = coalesce(p_payload->>'notes', notes)
  where id = p_id and user_id = current_user_id;

  if not found then
    raise exception 'Entrada nao encontrada';
  end if;
end;
$$;

revoke all on function public.update_income_entry(uuid, jsonb)
  from public, anon;

grant execute on function public.update_income_entry(uuid, jsonb)
  to authenticated;


create or replace function public.delete_income_entry(p_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
as $$
declare
  current_user_id uuid := auth.uid();
begin
  if current_user_id is null then
    raise exception 'Usuario nao autenticado';
  end if;

  delete from public.income_entries
  where id = p_id and user_id = current_user_id;

  if not found then
    raise exception 'Entrada nao encontrada';
  end if;
end;
$$;

revoke all on function public.delete_income_entry(uuid)
  from public, anon;

grant execute on function public.delete_income_entry(uuid)
  to authenticated;


-- ============================================================
-- EXPENSE ENTRIES
-- ============================================================

create or replace function public.create_expense_entry(p_payload jsonb)
returns uuid
language plpgsql
security definer
set search_path = ''
as $$
declare
  current_user_id uuid := auth.uid();
  new_id uuid;
begin
  if current_user_id is null then
    raise exception 'Usuario nao autenticado';
  end if;

  insert into public.expense_entries (
    user_id, description, amount, category, paid_at, payment_method, notes
  )
  values (
    current_user_id,
    p_payload->>'description',
    (p_payload->>'amount')::numeric,
    p_payload->>'category',
    (p_payload->>'paid_at')::date,
    p_payload->>'payment_method',
    p_payload->>'notes'
  )
  returning id into new_id;

  return new_id;
end;
$$;

revoke all on function public.create_expense_entry(jsonb)
  from public, anon;

grant execute on function public.create_expense_entry(jsonb)
  to authenticated;


create or replace function public.update_expense_entry(
  p_id uuid,
  p_payload jsonb
)
returns void
language plpgsql
security definer
set search_path = ''
as $$
declare
  current_user_id uuid := auth.uid();
begin
  if current_user_id is null then
    raise exception 'Usuario nao autenticado';
  end if;

  update public.expense_entries
  set
    description = coalesce(p_payload->>'description', description),
    amount = coalesce((p_payload->>'amount')::numeric, amount),
    category = coalesce(p_payload->>'category', category),
    paid_at = coalesce((p_payload->>'paid_at')::date, paid_at),
    payment_method = coalesce(p_payload->>'payment_method', payment_method),
    notes = coalesce(p_payload->>'notes', notes)
  where id = p_id and user_id = current_user_id;

  if not found then
    raise exception 'Gasto nao encontrado';
  end if;
end;
$$;

revoke all on function public.update_expense_entry(uuid, jsonb)
  from public, anon;

grant execute on function public.update_expense_entry(uuid, jsonb)
  to authenticated;


create or replace function public.delete_expense_entry(p_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
as $$
declare
  current_user_id uuid := auth.uid();
begin
  if current_user_id is null then
    raise exception 'Usuario nao autenticado';
  end if;

  delete from public.expense_entries
  where id = p_id and user_id = current_user_id;

  if not found then
    raise exception 'Gasto nao encontrado';
  end if;
end;
$$;

revoke all on function public.delete_expense_entry(uuid)
  from public, anon;

grant execute on function public.delete_expense_entry(uuid)
  to authenticated;
